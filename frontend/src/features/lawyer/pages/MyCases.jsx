...
export default function MyCases() {
  const { t } = useTranslation("lawyercases"); // Get the translation function
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm text-slate-600">{t("loadingCases")}</p>
        </div>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-500 text-sm">{t("errorFetching")}</p>
        </div>
      </div>
    );
  }
    ...
    <div className={`space-y-6 ${isRTL ? "lg:mr-[220px]" : "lg:ml-[220px]"}`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">
            {filteredCases.length} {filteredCases.length !== 1 ? t("cases") : t("case")}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          <button
            ...
            className={`px-2.5 py-1 text-[10px] cursor-pointer rounded transition-all ${
              !statusFilter
      ...
      {filteredCases.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6 text-center text-slate-500">
          <p className="text-sm font-medium text-slate-600">{t("noCasesFound")}</p>
          <p className="text-xs text-slate-500 mt-1">
            {search || statusFilter ? t("adjustFilters") : t("assignedCasesHere")}
          </p>
        </div>
      ) : (
            ...
            {filteredCases.map((c) => (
              <div key={c.id} className="border-b border-slate-200 p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 text-[10px]">
                  <div>
                    <span className="text-slate-500">{t("caseNumber")}:</span>
                  ...
                  <div>
                    <span className="text-slate-500">{t("hearing")}:</span>
                    <p className="text-slate-700">
                      {c.hearingDate ? formatDate(c.hearingDate) : <span className="text-slate-400 italic">{t("notSet")}</span>}
                    </p>
                  </div>
                </div>
                ...
                <button
                  onClick={() => openCase(c)}
                  className="w-full cursor-pointer p-1.5 bg-slate-100 text-[#0B1F3B] rounded transition-colors text-xs font-medium"
                >
                  {t("viewDetails")}
                </button>
              </div>
                  ...
                  <tr key={c.id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors duration-150">
                    ...
                    <td className="px-3 py-2 whitespace-nowrap hidden xl:table-cell">
                      {c.hearingDate ? (
                        ...
                        <span className="text-slate-400 text-[10px] italic">{t("notSet")}</span>
                      )}
                    </td>
                    <td className="px-3 py-2 whitespace-nowrap">
                      <div className="flex justify-center">
                        <button
                          onClick={() => openCase(c)}
                          className="p-1.5 cursor-pointer bg-slate-100 text-[#0B1F3B] rounded hover:bg-slate-200 transition-colors"
                          title={t("viewDetails")}
                        >
                          <FiEye size={14} />
                        </button>
      ...
      {selectedCase && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3">
          ...
          <div className="relative bg-white w-full max-w-5xl rounded-xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            {/* Header */}
            <div className="flex justify-between items-center px-6 py-4 bg-[#0B1F3B] text-white">
              <div>
              ...
              <button
                onClick={closeCase}
                className="p-2 rounded-lg cursor-pointer hover:bg-white/20 transition-colors"
              >
                <FiX size={20} />
              </button>
            </div>
              ...
              <div className="flex gap-6 overflow-x-auto">
                {["Details", "Documents", "Memorandum"].map((t) => (
                  <button
                    ...
                    className={`pb-2 cursor-pointer px-1 text-sm font-medium transition-all whitespace-nowrap ${
                      activeTab === t
            ...
            <div className="flex justify-end gap-2 px-6 py-4 border-t border-slate-200 sticky bottom-0 bg-white">
              <button
                onClick={closeCase}
                className="px-4 py-2 rounded-lg cursor-pointer border border-slate-300 text-sm text-slate-700
