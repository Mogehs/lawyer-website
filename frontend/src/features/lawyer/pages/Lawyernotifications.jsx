...
export default function LawyerNotifications() {
  const { t } = useTranslation("LawyerNotificationsPage");
  ...
  return (
    <div
     className={`space-y-6 ${isRTL ? "lg:mr-[220px]" : "lg:ml-[220px]"}`}
    >
      {/* Header - Compact */}
      <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between mb-3 gap-2 ">
        <div>
          ...
          <p className="text-[10px] sm:text-[18px] text-slate-600 mt-2">
            {filteredNotifications.length} {t('notification')}
      ...
      <div className="flex flex-col md:flex-row gap-2 mb-3">
        ...
        >
          ...
          <option value="New Assignment">{t('new_assignment')}</option>
          <option value="Upload Reminder">{t('upload_reminder')}</option>
          <option value="Hearing Reminder">{t('hearing_reminder')}</option>
          <option value="Ragab Feedback">{t('ragab_feedback')}</option>
        </select>
        ...
        >
          ...
          <option value="Cassation">{t('cassation')}</option>
        </select>
      </div>
